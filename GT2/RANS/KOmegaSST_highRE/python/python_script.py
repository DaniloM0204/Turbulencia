...
data5 = np.loadtxt("../postProcessing/sampleDict/2000/profile3.xy", skiprows=1)

data11 = np.loadtxt("./no_omega_WF/profile0_U_wallShearStress.xy", skiprows=0)
data22 = np.loadtxt("./no_omega_WF/profile1_U_wallShearStress.xy", skiprows=0)
...
fintnut = interpolate.interp1d(data2[:, 0], data2[:, 10], kind="linear")
...
wsm = np.sqrt(
    np.abs(fintwsx(xloc)) ** 2
    + np.abs(fintwsy(xloc)) ** 2
    + np.abs(fintwsz(xloc)) ** 2
)
...
plt.plot(ypn[1:], upn[1:], "-o", ms=5, label="OpenFOAM - WF - Current results")

# Omega WF off
plt.plot(ypn1[1:], upn1[1:], "-o", ms=5, label="OpenFOAM - No WF")
...
ws3 = (nu + fintnut(data5[:, 0])) * data5[:, 1] / 0.000400665
...
data4 = np.loadtxt("./ws_m2.csv", skiprows=1, delimiter=",")

# Compute cf using paraview data

# ws_pv=np.sqrt(data3[:,1]**2.+data3[:,2]**2.+data3[:,3]**2.)
# l_pv=data3[:,9]
ws_pv = np.sqrt(data3[:, 5] ** 2.0 + data3[:, 6] ** 2.0 + data3[:, 7] ** 2.0)
l_pv = data3[:, 11]
cf_pv = ws_pv / (0.5 * rho * U**2.0)

ws_pv1 = np.sqrt(data4[:, 5] ** 2.0 + data4[:, 6] ** 2.0 + data4[:, 7] ** 2.0)
l_pv1 = data4[:, 12]
cf_pv1 = ws_pv1 / (0.5 * rho * U**2.0)
...
plt.plot(data2[:, 0], cf_of, "-", label="OpenFOAM - WF")

plt.plot(data22[:, 0] + 0.1, cf_of1, "-", label="OpenFOAM - No WF")

plt.plot(l_pv + 0.1, cf_pv, label="Sampling using paraView")

plt.plot(data5[:, 0], cf_of3, label="Simplified calculation")

plt.plot(cf1[:, 0] + 0.1, cf1[:, 1], "-", label="CFL3D")

plt.plot(cf2[:, 0] + 0.1, cf2[:, 1], "-", label="FUN3D")
...
# Plot cf correlation and numerical solution of cf
...
# Mesh 1 takes WF
plt.plot(l_pv, cf_pv, label="Sampling using paraView - Mesh 1")

# Mesh 2 = NASA, resolves BL
plt.plot(l_pv1, cf_pv1, label="Sampling using paraView - Mesh 2")
