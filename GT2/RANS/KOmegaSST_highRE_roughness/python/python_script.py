...
data111 = np.loadtxt("./no_roughness/profile0_U_wallShearStress.xy",skiprows=0)
data222 = np.loadtxt("./no_roughness/profile1_U_wallShearStress.xy",skiprows=0)
...
fint11=interpolate.interp1d(data222[:,0],data222[:,4],kind='linear')
...
ws11 = np.sqrt(data111[:,4]**2 + data111[:,5]**2 + data111[:,6]**2)
um11 = np.sqrt(data111[:,1]**2 + data111[:,2]**2 + data111[:,3]**2)

#wsm=0.0012264945506493506
wsm11=np.abs(fint11(xloc))

utau11=np.sqrt(wsm11/rho)
ypn11=utau11*data111[:,0]/nu
upn11=um11/utau11
...
plt.plot(ypn[1:],upn[1:],'-o', ms=5, label='OpenFOAM - WF with roughness')

#Omega WF off
plt.plot(ypn1[1:],upn1[1:],'-o', ms=5, label='OpenFOAM - Omega WF disabled - No roughness')

#Omega no roughmess
plt.plot(ypn11[1:],upn11[1:],'-o', ms=5, label='OpenFOAM - WF - No roughness')
...
plt.plot(data2[:,0],cf_of,'-',label='OpenFOAM - WF with roughness')

plt.plot(data22[:,0],cf_of1,'-',label='OpenFOAM - Omega WF disabled - No roughness')
