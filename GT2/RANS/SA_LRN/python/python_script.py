...
data31 = np.loadtxt("./coarse_mesh_BC1/profile0_U_wallShearStress.xy",skiprows=0)
data42 = np.loadtxt("./coarse_mesh_BC1/profile1_U_wallShearStress.xy",skiprows=0)
...
fint2=interpolate.interp1d(data42[:,0],data42[:,4],kind='linear')
...
#ws2 = np.sqrt(data31[:,4]**2 + data31[:,5]**2 + data31[:,6]**2)
um2 = np.sqrt(data31[:,1]**2 + data31[:,2]**2 + data31[:,3]**2)

#wsm=0.0012264945506493506
wsm2=np.abs(fint2(xloc))

utau2=np.sqrt(wsm2/rho)
ypn2=utau2*data31[:,0]/nu
upn2=um2/utau2
...
plt.plot(ypn1[1:],upn1[1:],'-o', ms=5, label='OpenFOAM - SA with coarse mesh - fixedValue')

plt.plot(ypn2[1:],upn2[1:],'-o', ms=5, label='OpenFOAM - SA with coarse mesh - nutUSpaldingWallFunction')
...
ws42 = np.sqrt(data42[:,4]**2 + data42[:,5]**2 + data42[:,6]**2)
